...
const AppInfo = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setNoAuth(false)
      try {
        const doc = await db.collection("setting").doc("service").get()
        if (doc.exists) {
  ...
  const onSubmitClick = () => {
    try {
      db.collection("setting").doc("service").set({ text: textData })
      alert('서비스이용약관이 변경되었습니다.')
    } catch (e) {
  ...
  return (
    <div className={style.mainContainer}>
      <div className={`${style.container} ${style.quillContainer}`}>
        <Editor path={"setting/service"} handleChange={onTextChange} textData={textData} />
      </div>
