...
const TabSecurity = () => {
  ...
  const onSubmitClick = async () => {
    try {
      const reauthenticateUserResult = await reauthenticateUser(user.email, values.currentPassword)
      if (reauthenticateUserResult==="success") {
        const result = await updatePassword(values.currentPassword, values.newPassword, values.confirmNewPassword)
        if (result === true) {
          //성공------------
