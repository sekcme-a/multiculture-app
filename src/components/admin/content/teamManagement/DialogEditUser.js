...
const DialogEditUser = (props) => {
  const { user, userrole, teamName } = useAuth()
  ...
  const onButtonClick = async () => {
    try {
      let result
      if(props.mode==="add")
        result = await firebaseHooks.give_admin_role_with_user_uid(code, teamName)
      else
        result = await firebaseHooks.delete_admin_role_with_user_uid(code, teamName)
