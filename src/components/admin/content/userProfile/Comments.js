...
import styles from "styles/components/admin/content/comments.module.css"
...
import PercentageRadial from "src/components/charts/PercentageRadial"
...
const Comments = ({uid}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let tempComments = []
      if(doc.exists){
        db.collection("comments").doc(uid).collection("comment").orderBy("createdAt","desc").get().then((query)=>{
            query.forEach((item)=>{
  ...
  const onSubmitClick = async() => {
    if(inputComment==="" || inputComment===" "){
        ...
        db.collection("comments").doc(uid).get().then((doc)=>{
            if(doc.exists){
                db.collection("comments").doc(uid).update({
        ...
        await db.collection("comments").doc(uid).collection("comment").add({
    ...
    <Card>
      <CardContent>
        ...
        <CardContent className={styles.comments_container}>
            <h1 className={styles.title}>유저 메모</h1>
